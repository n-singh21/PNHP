...
bonap.changed.names = c("Carex paupercula","Marshallia pulchra","Primula meadia", "Viburnum trilobum","Viola appalachiensis")
...
db_ccvi_col <- c("RedOrange","BurntOrange","Goldenrod","Green","Gray")
...
for(i in 1:nrow(ccvi)){
  ...
  if(is.element(abr.sname,bonap.changed.names)==TRUE){ #checks for specific species that have name change
    bonap.disclaimer = "Note: The Pennsylvania Natural Heritage Program is currently updating its taxonomic standard, which sometimes recognizes different names than what is used in BONAP (the source of the range maps)."
    name.index = which(bonap.changed.names==abr.sname)
    abr.alt.name = bonap.alt.names[name.index] # identifying new name
    sep.names = unlist(strsplit(abr.alt.name,split=" +"))
    if(length(sep.names)==4){
      bonap.grep = paste0(sep.names[1],".*",sep.names[2],".*",sep.names[3],".*",sep.names[4])
    } else {
      bonap.grep = paste0(sep.names[1],".*",sep.names[2])
    }
  } else { # for the normal species
    bonap.disclaimer = ""
